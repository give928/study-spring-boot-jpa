package com.give928.springboot.jpa.api.member;

import com.give928.springboot.jpa.api.common.dto.ApiResultDto;
import com.give928.springboot.jpa.api.member.dto.*;
...
public class MemberApiController {
    ...
    public CreateMemberResponseDto saveMemberV2(@RequestBody @Valid CreateMemberRequestDto requestDto) {
        Long id = memberService.join(requestDto.getName(), requestDto.getCity(), requestDto.getStreet(), requestDto.getZipcode());
        return new CreateMemberResponseDto(id);
    }
    ...
    public ApiResultDto<List<MemberResponseDto>> membersV2() {
        List<Member> members = memberService.findMembers(); // 엔티티 -> DTO 변환
        List<MemberResponseDto> memberResponseDtos = members.stream()
                .map(MemberResponseDto::new)
                .collect(Collectors.toList());

        return new ApiResultDto<>(memberResponseDtos.size(), memberResponseDtos);
    }
}
